...
for name, player in sorted(players.items(), key=lambda t: list(t)[0]):
    ...
    lines.append('  Stages')
    for stage, results in sorted(player['stage'].items(), key=by_total, reverse=True):
        w = results['win']
        t = results['win'] + results['loss']
        lines.append(f'    {stage}: {t} game(s), {round(w/t*100, 2)}% win rate')
    lines.append('  Characters')
    for char, results in sorted(player['char'].items(), key=by_total, reverse=True):
        w = results['win']
        t = results['win'] + results['loss']
        lines.append(f'    {char}: {t} game(s), {round(w/t*100, 2)}% win rate')
    lines.append('  Opponents')
    for opp, results in sorted(player['opp'].items(), key=by_total, reverse=True):
        w = results['win']
        t = results['win'] + results['loss']
        lines.append(f'    {opp}: {t} game(s), {round(w/t*100, 2)}% win rate')

with open(f'team_data/{TEAM_NAME}', 'w', encoding='UTF-8') as f:
